...
const WebARRocksFaceThreeHelper = (function () {
  ...
  const _defaultSolvePnPImgPointsLabel = [
    'chin',
    'leftEarBottom',
    'rightEarBottom',
    'noseOuter',
  ...
  function init_PnPSolver(imgPointsLabels, objPointsPositions) {
    ...
    _computePose.imgPointsLMIndices = imgPointsLabels.map(function (
      label,
      ind
    ) {
      return _landmarks.labels.indexOf(label);
    });
    _computePose.objPoints = imgPointsLabels.map(function (label, ind) {
  ...
  function callbackReady(err, spec) {
    ...
    _three.preMatrix.scale(
      new Vector3(1.0, 1.0, 1.0).multiplyScalar(_spec.scale)
    );
  ...
  function process_faceSlot(detectState, slotIndex) {
    ...
    if (detectState.isDetected) {
      if (faceSlot.faceFollowerParent.visible) {
        // compute pose only if face follower is visible:
        let landmarks = null;
        if (!_landmarksStabilizers[slotIndex]) {
          _landmarksStabilizers[slotIndex] = stabilizer.instance(
            _landmarksStabilizerSpec || {}
          );
        }
  ...
  function compute_pose(landmarks, faceSlot) {
    ...
    const solved = _WEBARROCKSFACE.compute_pose(
      objectPoints,
      imgPointsPx,
      _focals[0],
      _focals[1]
  ...
  const that = {
    init: function (WEBARROCKSFACE, spec, landmarksStabilizerSpec) {
      ...
      console.log(
        'INFO in WebARRocksFaceThreeHelper - WEBARROCKSFACE.init spec = ',
        specLM
      );
    ...
    set_faceFollower: function (faceFollowerParent, faceFollower, faceIndex) {
      ...
      console.log(
        'INFO in WebARRocksFaceThreeHelper: set_faceFollower() for faceIndex = ',
        faceIndex
      );
    },
    ...
    update_threeCamera: function (sizing, threeCamera) {
      ...
      console.log(
        'INFO in WebARRocksFaceThreeHelper.update_threeCamera(): camera vertical estimated FoV is',
        fov,
        'deg'
      );
    ...
    update_video: function (video) {
      return new Promise(function (accept, reject) {
        _WEBARROCKSFACE.update_videoElement(
          video,
          function (newGlVideoTexture) {
